...
def _create_accounts(service, project, count, prefix='user'):
...
def _create_remaining_accounts(iam, project):
    ...
    while sa_count != 100:
        _create_accounts(iam, project, 100 - sa_count, prefix)
        sa_count = len(_list_sas(iam, project))


# Generate a random id
def _generate_id(prefix='fc-'):
    chars = '-abcdefghijklmnopqrstuvwxyz1234567890'
    return prefix + ''.join(choice(chars) for _ in range(5)) + choice(chars[1:])
        ...
        while current_key_dump is None or len(current_key_dump) != 100:
            ...
            if current_key_dump is None:
                ...
                for j in current_key_dump:
                    key_json = json.loads(j[1])
                    key_name = key_json['client_email'][:key_json['client_email'].find('iam')-1]
                    with open('%s/%s.json' % (path, key_name), 'w+') as f:
                        f.write(j[1])
